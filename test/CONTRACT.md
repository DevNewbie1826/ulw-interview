...
- `pendingThresholdCrossingConfirmation` is derived, never caller-trusted: replayed over scored rounds in round order (clear above threshold, clear on a user-kind answer, otherwise set when the prior effective ambiguity was above threshold or the flag already held). A stored value that disagrees rejects before the next event. When `closurePassed === true` the flag must be `false` (consumed by the confirmed audit). When `restateLoops > 0` the check is skipped because crossing consumption across correction cycles is not reconstructible from state.
