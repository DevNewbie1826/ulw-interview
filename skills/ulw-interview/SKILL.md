...
2. **`ask_topology`:** Propose 1-6 top-level components (Korean: `큰 덩어리`; English: `big chunks`), ask one confirmation question, then send `confirm_topology` with host-supplied `confirmedAt`.
...
EVERYTHING the user sees follows `plain-language.md`: plain Korean when the user speaks Korean, numbers explained in one plain line, and jargon only in agent-internal protocol text. Keep the user's own wording when it is clear. Silently self-proofread user-facing prose once before emitting it (spelling, spacing, grammar in the user's language); never announce or show the proofread pass.
