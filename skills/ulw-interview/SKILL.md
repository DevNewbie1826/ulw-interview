...
1. **`initialize`:** FIRST visible line must be exactly `Deep Interview threshold: {percent}% (source: {source})`. Then load `plain-language.md` NOW and keep its rules for the whole interview. Render a short welcome in the user's language.
